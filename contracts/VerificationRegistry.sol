...
contract VerificationRegistry is Ownable {
    ...
    function verifyUser(address user, bool phoneVerified, bool emailVerified, bool idVerified, bytes32 identityHash)
        external
        onlyOwner
    {
        require(!blacklist[user], "User is blacklisted");
        require(
            identityHashToUser[identityHash] == address(0) || identityHashToUser[identityHash] == user,
    ...
    function _calculateTrustScore(UserVerification memory verification) internal pure returns (uint256) {
        uint256 score = 10;
    ...
    function isUserVerified(address user, uint8 minLevel) external view returns (bool) {
        ...
        if (minLevel == 3) return verification.phoneVerified && verification.emailVerified && verification.idVerified;
